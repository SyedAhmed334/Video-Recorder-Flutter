import 'package:flutter/material.dart';

class XColors {
  XColors._();

  static Color primaryColor = const Color(0xffFF5757);
  static Color greyColor = const Color(0xFF808080);
  static Color secondaryColor = const Color(0xffF0F3FF);
  static Color textColor = const Color(0xff000000);
  static Color whiteColor = const Color(0xffFFFFFF);
  static Color hintTextColor = const Color(0xffBDBDBD);
  static Color iconInactiveColor = const Color(0xffADAFBB);
  static Color borderColor = const Color(0xffE8E6EA);
  static Color redColor = const Color(0xffff3333);
  static Color blackBackgroundColor = const Color(0xff000000);
}
